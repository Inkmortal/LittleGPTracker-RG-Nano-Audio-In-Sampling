...
| Phrase view | Existing core view plus sharp/flat note display and key-aware scale stepping | Notes, instruments, commands | Confirmed in sim | This is the most important editing screen; command columns need special audit. `note-spelling-workflow.rgsim` verifies `C#3`/`Db3` display switching, and `scale-key-workflow.rgsim` verifies keyed scale note edits through the normal `A + D-pad` shortcut. |
...
| Project/settings view | Existing core view plus musical context controls | Tempo, key, scale, note spelling, render, save/load | Confirmed in sim | Shows `Key:`, `Scale:`, and `Notes:` together without adding new button combos. `Key:` guides future scale-stepped note edits; it does not rewrite existing notes or block chromatic/outside notes. |

...
| Power menu overlay | RG Nano hardware plus simulator feature | Exit/debug without leaking input | Confirmed in sim | Text labels render in screenshots, and the power input remains isolated from tracker controls. Real RG Nano hardware uses Menu/Power as `KEY_POWER=q`; simulator uses `KEY_POWER=p`. |

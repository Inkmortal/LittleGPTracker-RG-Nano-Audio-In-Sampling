...
| Context/helper overlay | Universal `R + Select` overlay on main tracker views | Fast orientation on a screen with no room for permanent help | Confirmed in sim | Page 1 shows current screen name, the Song -> Chain -> Phrase minimap, main exits, and primary edit gesture. `Up`/`Down` switches to a command page with inputs for the current screen. It blocks normal tracker inputs while open and closes with `R + Select`, leaving raw `Select` free for focused producer actions. |
